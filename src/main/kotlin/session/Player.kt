...
class Player(playerData: PlayerData) {
    ...
    fun getBountyFormatted(ramp:Float) = "${addCommas(min(getBounty()*ramp, getBounty().toFloat()).toInt().toString())} W$"
    ...
    fun changeBounty(amount:Int) {
        change = amount
        bounty += amount
        if (getChange()>0) changeCol = ColF(changeColDelay*2, changeColInterval, Vec4(1,1,1,0), Vec4(0.2,0.8,0.2,1))
        else changeCol = ColF(changeColDelay, changeColInterval, Vec4(1,1,1,0), Vec4(0.8,0.2,0.2,1))
    ...
    fun getRatingColor(): Vec4 {
        ...
        if (getMatchesWon() >= 4 && getRating() >= 0.6f) color  = Vec4(0.98, 0.64, 0.10, 1) // B+
